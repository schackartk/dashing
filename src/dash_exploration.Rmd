...
```{r, echo = FALSE}
paste0("Total Number of Dashes: ", nrow(df_deliveries))
paste0("Total Earnings: ", sum(df_deliveries$total))
```
...
```{r}
df_dashes %>% summarize(tot_hrs = sum(total_time),
                        tot_pay = sum(earnings),
                        avg_hrly = tot_pay/tot_hrs)
```
