...
df_dashes <- df_dashes %>% mutate(
  ...
  total_time = round(as.double(difftime(end, start, units = "hours"), digits = 2)))
...
# I want to add is SD or 95% CI on the below stats,
...
```{r}
weekday_summary %>% ggplot(aes(x = day, y = mean_tip, fill = mean_tip)) +
  geom_col() +
  labs(x = "",
       y = "Average Tip ($)") +
  theme_light()
